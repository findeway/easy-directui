...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    LPBYTE pData = NULL;
    DWORD dwSize = 0;
	bool bIsIcon = _tcsstr(bitmap.m_lpstr,L".ico") != NULL;
	HBITMAP hIconBitmap = NULL;
	if(bIsIcon)
	{
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		sFile += bitmap.m_lpstr;
		HICON hIcon = (HICON)::LoadImage(CPaintManagerUI::GetInstance(),LPCTSTR(sFile),IMAGE_ICON, 0, 0, LR_DEFAULTSIZE);
	/*	ICONINFO iconinfo;
		GetIconInfo(hIcon, &iconinfo);
		hIconBitmap = iconinfo.hbmColor;
		TImageInfo* data = new TImageInfo;
		data->hBitmap = hIconBitmap;
		data->nX = iconinfo.xHotspot;
		data->nY = iconinfo.yHotspot;
		...
		bool bHole, bool bTiledX, bool bTiledY)
{
	...
	if( sImageName.Find(_T(".ico")) >= 0)
	{
		//ico执行特殊处理，duilib无法绘制ico文件
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		if(sFile.Find(_T(":")) < 0)
		{
			sFile += sImageName;
		...
		if(_taccess(LPCTSTR(sFile),0) == 0)
		{
			HICON hIcon = (HICON)::LoadImage(NULL,LPCTSTR(sFile),IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
			DrawIconEx(hDC,rc.left,rc.top,hIcon,rc.right-rc.left,rc.bottom-rc.top,0,NULL,DI_NORMAL);
			return true;
		}
	}
