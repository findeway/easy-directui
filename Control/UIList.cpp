...
void CListBodyUI::SetScrollPos(SIZE szPos)
{
    ...
    if( cx == 0 && cy == 0 ) return;
	//必须先更新listheader,因为此时列表控件的columns区域
	//否则在对listelement执行setpos时会导致listbody与listheader不对齐的问题
	if( cx != 0 && m_pOwner ) {
...
CListRichElementUI::CListRichElementUI()
{
}

CListRichElementUI::~CListRichElementUI(void)
{
}

void CListRichElementUI::DoEvent(TEventUI& event)
{
	if (event.Type == UIEVENT_DBLCLICK)
	{
		if (IsEnabled())
...
CListRichElementUI* CListRichElementUI::CreateControl(DuiLib::CDuiString strSkinFile)
{
	DuiLib::CDialogBuilder dlgBuilder;
	CListRichElementUI* element = static_cast<CListRichElementUI*>(dlgBuilder.Create(LPCTSTR(strSkinFile), UINT(0)));
	element->SetVisible(true);
	return element;
}

void CListRichElementUI::Notify(TNotifyUI& msg)
{
}

void CListRichElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	if (!::IntersectRect(&m_rcPaint, &rcPaint, &m_rcItem)) return;
	DrawItemBk(hDC, m_rcItem);
	CContainerUI::DoPaint(hDC, rcPaint);
...
void CListRichElementUI::SetPos(RECT rc)
{
	if (m_pOwner == NULL) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	CControlUI::SetPos(rc);
	if(pInfo->nColumns == 0)
	{
		...
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			...
			if( !pControl->IsVisible() ) continue;
			RECT itemRect = rc;
			itemRect.left = rc.left + (double)(rc.right - rc.left) / m_items.GetSize() * (it);
			itemRect.right = itemRect.left + (double)(rc.right - rc.left) / m_items.GetSize();
	...
	for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		if( sz.cy > pControl->GetMaxHeight() ) sz.cy = pControl->GetMaxHeight();
		//更新子控件区域
		RECT rcCtrl = { iPosX + rcPadding.left , rc.top + rcPadding.top, iPosX + rcPadding.left + rcPadding.right + pInfo->rcColumn[it2].right - pInfo->rcColumn[it2].left, rc.top + rcPadding.top + sz.cy};
		pControl->SetPos(rcCtrl);
		//计算下一个子控件的起始坐标
		iPosX += pInfo->rcColumn[it2].right - pInfo->rcColumn[it2].left + rcPadding.left + rcPadding.right; 
...
LPVOID CListRichElementUI::GetInterface( LPCTSTR pstrName )
{
	if( _tcscmp(pstrName, DUI_CTR_LISTRICHELEMENT) == 0 ) return static_cast<CListRichElementUI*>(this);
